...
lazy_static::lazy_static! {
    ...
    pub static ref LOGIN_USERNAME: Arc<Mutex<String>> = Default::default();
...
lazy_static::lazy_static! {
    // pub static ref LOGIN_USERNAME: Arc<Mutex<String>> = Default::default();
    pub static ref USERNAME_CHANGED: Arc<Mutex<bool>> = Arc::new(Mutex::new(false));
}

#[no_mangle]
pub extern "C" fn set_login_username(username: *const hbb_common::libc::c_char) {
    let username_str = unsafe { std::ffi::CStr::from_ptr(username).to_string_lossy().into_owned() };
    let old_username = LOGIN_USERNAME.lock().unwrap().clone();
    
    if username_str != old_username {
        log::debug!("Username changed from '{}' to '{}'", old_username, username_str);
        
        *LOGIN_USERNAME.lock().unwrap() = username_str;
        *USERNAME_CHANGED.lock().unwrap() = true;
    }
}
...
pub fn hostname() -> String {
    let login_username = LOGIN_USERNAME.lock().unwrap().clone();
    let system_hostname = {
        #[cfg(not(any(target_os = "android", target_os = "ios")))]
        {
            #[allow(unused_mut)]
    ...
    if login_username.is_empty() {
        ...
        format!("[{}] {}", login_username, system_hostname)
    }
}
...
pub fn get_sysinfo() -> serde_json::Value {
    ...
    let hostname = if login_username.is_empty() {
        system_hostname
    } else {
        format!("[{}] {}", login_username, system_hostname)
    };
...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    "http://64.81.112.194:50004".to_owned()
}
